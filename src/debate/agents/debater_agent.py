...
class DebaterAgent(BaseAgent):
    ...
    def build_prompt(self, phase: Phase) -> str:
        ...
        Includes - always - the motion, the stance, the phase, and a
        clear stance instruction. Opponent's last message is included
        for ``argument`` and ``closing`` phases (it has no meaning
        for the opening turn). Selected context and recorded tool
        results are appended when present.
        """
        sections: list[str] = [
    ...
    def _stance_instruction(self) -> str:
        if self.STANCE == "pro":
            return (
                ...
                "stance for the entire debate."
            )
        return "You argue AGAINST the motion. Stay strictly in this stance for the entire debate."
